...
__all__ = ['VaultutilpyError' 'MissingEnvVar', 'AuthTokenRetrieval']
...
def in_cluster_client():
    """
    in_cluster_client returns a vault (hvac) API client using
    environment variables passed to pods within a kubernetes cluster
    """
    for envvar in ["VAULT_ADDR", "VAULT_AUTH_PATH", "VAULT_ROLE"]:
        if os.environ.get(envvar, False):
...
def in_cluster_secret(path, field):
    """
    in_cluster_secret is a helper function to retrieve a
    secret from vault from within kubernetes
    """
